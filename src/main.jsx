...
function TopNav({ t, lang, setLang }) {
  ...
  const hrefs = ['#home', '#about', '#catalog', '#diploma-access', '#dashboard', '#login', '#certificate'];
...
function DiplomaAccess({ lang }) {
  const rtl = lang === 'ar';
  return <section id="diploma-access" className="section diploma-access"><div><p className="eyebrow">{rtl ? 'بوابة الدبلوم' : 'Diploma access'}</p><h2>{rtl ? 'دبلوم تمريض القسطرة القلبية' : 'CCL Nursing Diploma'}</h2><p className="section-lead">{rtl ? 'تم فصل بيانات الدبلوم التفصيلية عن الواجهة الرئيسية. استخدم هذه الخانة فقط للوصول إلى صفحة الدبلوم المستقلة داخل نفس الموقع.' : 'Detailed diploma information is separated from the homepage. Use this access box only to open the standalone diploma page within the same site.'}</p></div><a className="enroll-button diploma-access-button" href="./program.html"><GraduationCap /> {rtl ? 'الدخول إلى صفحة الدبلوم' : 'Open diploma page'}</a></section>;
}
